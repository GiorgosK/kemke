...
namespace Drupal\incoming_api\Service;
...
/**
 * Manages chunked uploads for incoming document files.
 */
final class ChunkUploadManager {
   ...
   */
  private const STORE_ID = 'incoming_api_chunk_uploads';

  /**
   * The base directory for temporary upload chunks.
   */
  private const TEMPORARY_DIRECTORY = 'temporary://incoming_api';
  ...
  public function completeUpload(string $uploadId, ?int $uid): FileInterface {
    ...
    if (isset($metadata['total_size']) && $metadata['total_size'] > 0 && $metadata['received_size'] !== $metadata['total_size']) {
      $this->logger->warning('incoming_api chunk upload size mismatch for @id. Expected @expected bytes, received @received bytes.', [
        '@id' => $uploadId,
