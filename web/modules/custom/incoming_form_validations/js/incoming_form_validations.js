(function (Drupal) {
  ...
  const updateButtonState = (button, requirement) => {
    const hasEmpty = requirement.emptyFields.some((config) => {
      ...
      if (field) {
        ...
        if (config.type === 'select2' && window.jQuery) {
          ...
          if (typeof $field.on === 'function') {
            $field.on('select2:select select2:unselect select2:clear select2:close select2:opening select2:closing select2:open', () => {
              refresh();
            });
          }
        }
