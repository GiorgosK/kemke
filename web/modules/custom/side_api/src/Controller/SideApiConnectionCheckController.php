...
final class SideApiConnectionCheckController extends ControllerBase {
  ...
  public function build(string $environment = 'dev'): array {
    ...
    $check_uid = (int) ($request?->query->get('check_uid', '0') ?? '0');
      ...
      'intro' => [
        ...
        'context' => [
          ...
          '#items' => [
            ...
            'Drupal user UID check: ' . ($check_uid > 0 ? (string) $check_uid : '(not checked)'),
        ...
        'links' => [
          ...
          '#items' => [
            ...
            Link::fromTextAndUrl($this->t('Check Drupal user 33'), Url::fromRoute('side_api.connection_check', ['environment' => $environment], [
              'query' => [
                'login_timeout' => 15,
                'login_attempts' => 1,
                'probe_timeout' => 15,
   ...
   */
  private function runDiagnostics(string $environment, string $baseUrl, string $adminUser, string $adminPass, string $appUser, string $appPass, array $lookupUsers, float $loginTimeout, int $loginAttempts, float $probeTimeout, string $signerUser = '', string $signerGroupId = '', bool $checkSupervisors = FALSE, int $checkUid = 0): array {
    ...
    try {
      ...
      if ($checkUid > 0) {
        $results[] = $this->runDrupalUidCheck($checkUid, $environment, $baseUrl, $jar, $probeTimeout, $started, $loginElapsed);
      }
  ...
  /**
   * Check one Drupal user by UID against SIDE relations.
   *
   * @return array{check:string, environment:string, base_url:string, status:string, login_elapsed:string, probe_elapsed:string, total_elapsed:string, probe_user:string, probe_timeout:string, probe_response_keys:string, probe_summary:string, failure_stage:string, exception_class:string, previous_exception:string, http_status:string, details:string}
   */
  private function runDrupalUidCheck(int $uid, string $environment, string $baseUrl, \GuzzleHttp\Cookie\CookieJarInterface $jar, float $probeTimeout, float $started, float $loginElapsed): array {
    $account = $this->diagnosticEntityTypeManager->getStorage('user')->load($uid);
    if (!$account instanceof UserInterface) {
      return $this->buildDiagnosticRow('user-uid:' . $uid, $environment, $baseUrl, 'User not found', sprintf('%.3fs', $loginElapsed), '-', sprintf('%.3fs', microtime(TRUE) - $started), '-', $probeTimeout, '-', '-', '-', '-', '-', '-', 'No Drupal user exists with UID ' . $uid . '.');
    }

    $environmentKey = $environment === 'live' ? 'live' : 'test';
    return $this->runDrupalSupervisorCheck($account, $environment, $environmentKey, $baseUrl, $jar, $probeTimeout, $started, $loginElapsed, 'user-uid:' . $uid);
  }
   ...
   * @return array{check:string, environment:string, base_url:string, status:string, login_elapsed:string, probe_elapsed:string, total_elapsed:string, probe_user:string, probe_timeout:string, probe_response_keys:string, probe_summary:string, failure_stage:string, exception_class:string, previous_exception:string, http_status:string, details:string}
   */
  private function runDrupalSupervisorCheck(UserInterface $account, string $environment, string $environmentKey, string $baseUrl, \GuzzleHttp\Cookie\CookieJarInterface $jar, float $probeTimeout, float $started, float $loginElapsed, ?string $checkName = NULL): array {
    ...
    $configuredGroupId = $configured['groupid'] ?: $configured['id'];
    $checkName = $checkName ?? 'supervisor:' . $account->id();
    $detailsPrefix = sprintf(
      'uid=%d label=%s environment_key=%s drupal_docutracks_id=%s configured_id=%s configured_groupid=%s effective_ToSign=%s',
      (int) $account->id(),
      $account->label(),
      $environmentKey,
    ...
    if ($username === '') {
      return $this->buildDiagnosticRow($checkName, $environment, $baseUrl, 'No username', sprintf('%.3fs', $loginElapsed), '-', sprintf('%.3fs', microtime(TRUE) - $started), '-', $probeTimeout, '-', '-', '-', '-', '-', '-', $detailsPrefix . ' | field_docutracks_username is empty.');
    }

    if ($configuredGroupId === '') {
      return $this->buildDiagnosticRow($checkName, $environment, $baseUrl, 'No configured group', sprintf('%.3fs', $loginElapsed), '-', sprintf('%.3fs', microtime(TRUE) - $started), $username, $probeTimeout, '-', '-', '-', '-', '-', '-', $detailsPrefix . ' | field_dt_config does not contain a groupid or id for this environment.');
    }
    ...
    try {
      ...
      $decision = $this->buildSupervisorGroupDecision($configuredGroupId, $groupIds, $idMismatch);
      $status = $decision['status'];
      $details = sprintf(
        '%s side_user_id=%s SIDE_possible_ToSign_groups=%s suggested_ToSign_candidates=%s decision=%s suggested_field_dt_config_groupid=%s',
        $detailsPrefix,
        $sideUserId !== '' ? $sideUserId : '-',
        $this->formatRelationGroups($relationGroups) ?: '(none found)',
        $groupIds !== [] ? implode(', ', $groupIds) : '(none found)',
        $decision['message'],
        $decision['suggested_groupid'] !== '' ? $decision['suggested_groupid'] : '-',
      );
   ...
   *
   * @return array{status:string, message:string, suggested_groupid:string}
   */
  private function buildSupervisorGroupDecision(string $effectiveGroupId, array $sideGroupIds, bool $idMismatch): array {
    if ($idMismatch) {
      return [
    ...
    if ($effectiveGroupId !== '' && in_array($effectiveGroupId, $sideGroupIds, TRUE)) {
      return [
        'status' => 'OK',
        'message' => sprintf('Configured ToSign.Id %s is present in SIDE relation groups. No user config change needed.', $effectiveGroupId),
        'suggested_groupid' => '',
      ];
    }
    ...
    if (count($sideGroupIds) === 1) {
      ...
      return [
        ...
        'message' => sprintf('Configured ToSign.Id is not valid. SIDE reports exactly one possible group, so set field_dt_config department groupid to %s.', $candidate),
        'suggested_groupid' => (string) $candidate,
      ];
    }
